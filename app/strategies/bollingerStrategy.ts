...
const bollingerStrategy: BollingerStrategy = {
  ...
  analyzeEntry(data: CandlestickData<Time>[], index: number): 'buy' | null {
    ...
    console.log({
      ...
      'MA600 하락 여부': isMA600Falling ? '❌ 매수 제한' : '✅ 매수 가능',
      '조건 5 (MA60 < MA600)': isBelow600 ? '✅' : '❌' + (useFifthCondition ? '' : ' [비활성화됨]'),
      '최종 판정': (ma240UpCount >= 5 && isAbove120 && isAbove240 && isMA600Upward && !isMA600Falling && (isBelow600 || !useFifthCondition)) ? '✅ 매수 신호 발생!' : '❌ 매수 조건 불충족'
    });
    ...
    if (isMA600Falling) {
      console.log('\n=== ❌ MA600 하락으로 매수 제한 ===');
      console.log('MA600이 하락 중이므로 매수하지 않습니다.');
      return null;
    }
  ...
  analyzeExit(data: CandlestickData<Time>[], index: number, position: 'buy', entryPrice: number): boolean {
    ...
    console.log('매도 조건:', {
      ...
      'MA60이 MA600 위': isAbove600,
      '현재 수익률': profitPercent.toFixed(2) + '%'
    });

    // 매도 시그널 생성 - 기본 조건
    if (ma240DownCount >= 5 && isBelow120 && isBelow240 && isMA600Upward && isAbove600) {
      console.log('\n=== 매도 조건 충족 여부 ===');
