...
type MASettingKey = 'sixty' | 'oneTwenty' | 'twoForty' | 'threeHundredSixty' | 'threeHundred' | 'nineHundred' | 'twelveHundred';
...
const ChartSettings: React.FC<ChartSettingsProps> = ({
  ...
  const storeShowMA = useUpbitStore((state) => state.showMA);
  const toggleMAStore = useUpbitStore((state) => state.updateShowMA);

  useEffect(() => {
    setMounted(true);
  }, []);

  const handleToggle = (key: MASettingKey) => {
    const newState = {
      ...showMA,
      [key]: !showMA[key]
    };
    updateShowMA(newState);
    toggleMAStore(key);
  };
