  ...
  const findCrossPoints = (
    ...
    for (let i = 1; i < shortEMA.length; i++) {
      ...
      if (
        ...
        orderBookAnalysis.isStableSpread &&
        orderBookAnalysis.isPotentialUp &&
        !currentPosition
      ) {
        ...
        crossPoints.push({
          ...
          position: 'buy',
          value: currShort,
          reason: '매수 신호 (골든크로스, RSI, 스토캐스틱, 거래량, 피보나치/호가창)'
        });
      }
      ...
      if (currentPosition) {
        ...
        else if (sellConditions.some(cond => cond) && orderBookAnalysis.isPotentialDown) {
          crossPoints.push({
            time: shortEMA[i].time,
            position: 'sell',
            value: currShort,
            reason: '매도 신호 (데드크로스, RSI, 스토캐스틱, 호가창)'
          });
          currentPosition = null;
        }
      }
