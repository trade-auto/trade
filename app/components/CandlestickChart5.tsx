...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  const loadChartData = useCallback(async () => {
    ...
    try {
      ...
      // 특정 시간대의 데이터 로그 출력
      threeEMAData.forEach((ema, index) => {
        const time = ema.time;
        if (time === '04:40' || time === '04:45') { // 예시 시간대
          console.log(`Time: ${time}, 30MA: ${ema.value}, 40MA: ${sixEMAData[index].value}, 60MA: ${twentyEMAData[index].value}`);
        }
      });
  ...
  // RSI 계산 함수 추가
  const calculateRSI = (data: ExtendedCandlestickData[], period: number): number[] => {
    const gains: number[] = [];
    const losses: number[] = [];
  ...
  const findCrossPoints = (thirtyEMA: LineData<Time>[], fortyEMA: LineData<Time>[], sixtyEMA: LineData<Time>[], rsi: number[]): CrossPoint[] => {
    ...
    for (let i = 1; i < thirtyEMA.length; i++) {
      ...
      const currentRSI = rsi[i - 1]; // RSI는 EMA와 인덱스가 다를 수 있음

      // RSI가 30 이하에서 반등하는 경우에만 매수
      if (currThirty > currSixty && currForty > currSixty && currentRSI > 30 && lastAction !== 'buy') {
        if (prevThirty <= prevForty && currThirty > currForty) {
          crossPoints.push({
