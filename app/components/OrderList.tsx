...
export function OrderList({ mode, onSelectOrder = () => {} }: OrderListProps) {
  ...
  const loadOrderList = async () => {
    ...
    if (uuids.length === 0) {
      setError('UUID를 입력해주세요. (여러 개인 경우 쉼표로 구분)');
      return;
    }
    ...
    try {
      setIsLoading(true);
      setError(null);
      const data = await getOrderList(uuids);
      setOrders(data);
      setIsInputVisible(false); // 조회 성공 시 입력 폼 숨기기
    } catch (error: any) {
        ...
        <div className="bg-gray-800 p-4 rounded-lg mb-4">
          <div className="flex space-x-4">
            <input
              ...
              placeholder="UUID를 입력하세요 (여러 개인 경우 쉼표로 구분)"
          ...
          <p className="text-gray-400 text-sm mt-2">
            여러 주문을 조회하려면 UUID를 쉼표(,)로 구분하여 입력하세요.
          </p>
        </div>
      )}
