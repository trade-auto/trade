...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  // MA 기간 설정을 위한 상태 추가
  const [threePeriod, setThreePeriod] = useState<number>(3);
  const [sixPeriod, setSixPeriod] = useState<number>(6);
  const [twentyPeriod, setTwentyPeriod] = useState<number>(20);
  
  // MA 기간 변경 핸들러
  const handleMAChange = (type: 'three' | 'six' | 'twenty', value: number) => {
    if (type === 'three') {
      setThreePeriod(value);
    } else if (type === 'six') {
      setSixPeriod(value);
    } else if (type === 'twenty') {
      setTwentyPeriod(value);
    }
    ...
    if (candleSeriesRef.current && threeEMASeriesRef.current && sixEMASeriesRef.current && twentyEMASeriesRef.current) {
      ...
      const threeEMAData = calculateEMA(candleData, type === 'three' ? value : threePeriod);
      const sixEMAData = calculateEMA(candleData, type === 'six' ? value : sixPeriod);
      const twentyEMAData = calculateEMA(candleData, type === 'twenty' ? value : twentyPeriod);
  ...
  const loadChartData = useCallback(async () => {
    ...
    try {
      ...
      // 이동평균 계산
      const threeEMAData = calculateEMA(candleData, threePeriod);
      const sixEMAData = calculateEMA(candleData, sixPeriod);
      const twentyEMAData = calculateEMA(candleData, twentyPeriod);
  ...
  // 크로스 포인트 찾기 함수 수정
  const findCrossPoints = (threeEMA: LineData<Time>[], sixEMA: LineData<Time>[], twentyEMA: LineData<Time>[]): CrossPoint[] => {
    const crossPoints: CrossPoint[] = [];
    let lastAction: 'buy' | 'sell' | null = null; // 마지막 액션을 추적
    
    for (let i = 1; i < threeEMA.length; i++) {
      const prevThree = threeEMA[i - 1].value;
      const prevSix = sixEMA[i - 1].value;
      const prevTwenty = twentyEMA[i - 1].value;
      const currThree = threeEMA[i].value;
      const currSix = sixEMA[i].value;
      const currTwenty = twentyEMA[i].value;
      
      // MA3과 MA6이 MA20 이상일 때만 매수
      if (currThree > currTwenty && currSix > currTwenty) {
        // 골든크로스 (3MA가 6MA를 상향돌파)
        if (prevThree <= prevSix && currThree > currSix && lastAction !== 'buy') {
          crossPoints.push({
            time: threeEMA[i].time,
            position: 'buy',
            value: currThree,
          });
      ...
      // MA3과 MA6이 MA20 이하로 떨어질 때 매도
      else if (currThree < currTwenty && currSix < currTwenty && lastAction !== 'sell') {
        crossPoints.push({
          time: threeEMA[i].time,
          position: 'sell',
          value: currThree,
      ...
      // 데드크로스 (3MA가 6MA를 하향돌파)
      else if (prevThree >= prevSix && currThree < currSix && lastAction !== 'sell') {
        crossPoints.push({
          time: threeEMA[i].time,
          position: 'sell',
          value: currThree,
    ...
    <div className="w-full min-h-screen p-4 bg-[#1e1e1e] rounded-lg">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="bg-gray-800 p-4 rounded-lg">
          <div className="text-gray-400 text-sm mb-2">3분 이동평균선 기간</div>
          <div className="flex items-center space-x-4">
            <input
              ...
              max="30"
              value={threePeriod}
              onChange={(e) => handleMAChange('three', parseInt(e.target.value))}
              className="flex-1"
            />
            <div className="text-white font-bold w-12 text-center">{threePeriod}</div>
          </div>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg">
          <div className="text-gray-400 text-sm mb-2">6분 이동평균선 기간</div>
          <div className="flex items-center space-x-4">
            <input
              type="range"
              min="5"
              max="50"
              value={sixPeriod}
              onChange={(e) => handleMAChange('six', parseInt(e.target.value))}
              className="flex-1"
            />
            <div className="text-white font-bold w-12 text-center">{sixPeriod}</div>
          </div>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg">
          <div className="text-gray-400 text-sm mb-2">20분 이동평균선 기간</div>
          <div className="flex items-center space-x-4">
            <input
              type="range"
              min="10"
              max="100"
              value={twentyPeriod}
              onChange={(e) => handleMAChange('twenty', parseInt(e.target.value))}
              className="flex-1"
            />
            <div className="text-white font-bold w-12 text-center">{twentyPeriod}</div>
          </div>
        </div>
      </div>
