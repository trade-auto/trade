...
const CandlestickChart: React.FC<CandlestickChartProps> = (props) => {
  ...
  if (chartType === 'seconds/60') {
    ...
    useEffect(() => {
      ...
      const realtimeInterval = 5000; // 5초마다 업데이트
  ...
  const updateMovingAverages = useCallback((data: ExtendedCandlestickData[]) => {
    ...
    // 각 MA 계산
    const ma60Data = calculateEMA(data, 60);
    const ma120Data = calculateEMA(data, 120);
    const ma240Data = calculateEMA(data, 240);
    const ma360Data = calculateEMA(data, 360);
    const ma300Data = calculateEMA(data, 300);
    const ma900Data = calculateEMA(data, 900);
    
    // 차트에 적용
    if (sixtyEMASeriesRef.current) {
      sixtyEMASeriesRef.current.setData(ma60Data);
    }
    if (oneTwentyEMASeriesRef.current) {
      oneTwentyEMASeriesRef.current.setData(ma120Data);
    }
    if (twoFortyEMASeriesRef.current) {
      twoFortyEMASeriesRef.current.setData(ma240Data);
    }
    if (threeHundredSixtyEMASeriesRef.current) {
      threeHundredSixtyEMASeriesRef.current.setData(ma360Data);
    }
    if (threeHundredEMASeriesRef.current) {
      threeHundredEMASeriesRef.current.setData(ma300Data);
    }
    if (nineHundredEMASeriesRef.current) {
      nineHundredEMASeriesRef.current.setData(ma900Data);
    }
  }, []);
  ...
  const updateRealtimeData = useCallback(async () => {
    ...
    try {
      // API에서 최신 데이터 가져오기
      const now = new Date();
      const secondsAgo = new Date(now.getTime() - 60 * 1000); // 60초 전
      
      console.log('실시간 데이터 업데이트 중...', secondsAgo.toISOString(), now.toISOString());
      
      // Upbit API 호출하여 최신 데이터 가져오기
      const response = await axios.get('https://api.upbit.com/v1/candles/seconds/60', {
        params: {
          market: symbol,
          count: 10, // 최대 10개의 최신 캔들만 가져옴
          to: now.toISOString()
        }
      });
      ...
      if (response.data && response.data.length > 0) {
        // 데이터 변환 (Upbit API 응답 형식 -> 차트 형식)
        const newCandles = response.data.map((candle: any) => ({
        ...
        if (newCandles.length > 0) {
          ...
          // 새 캔들 데이터 추가 (중복 제거)
          newCandles.forEach((newCandle: ExtendedCandlestickData) => {
            const existingIndex = updatedData.findIndex(candle => candle.time === newCandle.time);
            if (existingIndex >= 0) {
              // 기존 데이터 업데이트
          ...
          // 데이터 정렬 (시간 기준)
          updatedData.sort((a, b) => Number(a.time) - Number(b.time));
          ...
          if (volumeSeriesRef.current) {
            const volumeData = updatedData.map(d => ({
              time: d.time,
              value: d.volume || 0,
              color: d.close >= d.open ? '#26a69a' : '#ef5350',
            }));
          ...
          if (updatedData.length >= 900) {
            try {
              const result = bollingerStrategy.analyze(updatedData, {
                realtime: true,
                lastProcessedIndex: lastAnalysisResult?.lastProcessedIndex || 0,
                currentPosition: lastAnalysisResult?.currentPosition || null,
                lastTradeId: lastAnalysisResult?.lastTradeId || null,
              ...
              if (result.signals && result.signals.length > 0) {
                const newMarkers = createTradeMarkers(result.signals);
                setMarkers(prevMarkers => [...prevMarkers, ...newMarkers]);
                
                // 마커 플러그인에 마커 추가
                if (markerPluginRef.current) {
                  try {
                    // @ts-ignore - 타입 정의 문제 무시
                    markerPluginRef.current.add(newMarkers);
                  } catch (e) {
              ...
              console.error('볼린저 전략 분석 중 오류:', error);
            }
          }
          
          console.log(`실시간 업데이트 성공: ${newCandles.length}개 캔들 추가/업데이트`);
        }
  ...
  }, [isRealtimeAPIEnabled, chartType, symbol, allData, updateMovingAverages, lastAnalysisResult]);
