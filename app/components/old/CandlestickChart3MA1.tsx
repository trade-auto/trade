...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  // MA 기간 변경 핸들러
  const handleMAChange = (type: 'three' | 'six', value: number) => {
    if (type === 'three') {
    ...
    // 차트 데이터 업데이트
    if (candleSeriesRef.current && threeEMASeriesRef.current && sixEMASeriesRef.current) {
      ...
      // 크로스 포인트 업데이트
      const crossPoints = findCrossPoints(threeEMAData, sixEMAData);
  ...
  // 크로스 포인트 찾기 함수
  const findCrossPoints = (threeEMA: LineData<Time>[], sixEMA: LineData<Time>[]): CrossPoint[] => {
