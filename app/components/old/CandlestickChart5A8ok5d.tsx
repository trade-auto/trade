 ...
 */
const getInitialDateRange = (chartType: string): DateRange => {
  const now = new Date();
  ...
  const numericType = parseInt(chartType);
  if (numericType === 240) { // 일봉
    return {
      startDate: new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000), // 1년 전
      endDate: now,
    };
  } else if (numericType === 7200) { // 월봉
    return {
      startDate: new Date(
        now.getFullYear() - 5,
        now.getMonth(),
...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  const findCrossPoints = (thirtyEMA: LineData<Time>[], fortyEMA: LineData<Time>[], sixtyEMA: LineData<Time>[]): CrossPoint[] => {
    ...
    // 데이터 안정화를 위한 시작 시간 설정 (첫 데이터 + 60초)
    const stabilizationTime = (thirtyEMA[0].time as number) + 60; // 30초에서 60초로 변경
  ...
  // CSV 저장 함수 추가
  const saveToCSV = (candleData: ExtendedCandlestickData[]) => {
    // CSV 헤더
    const header = 'timestamp,open,high,low,close,volume\n';
    
    // 데이터를 CSV 형식으로 변환
    const csvContent = candleData.map(candle => {
      const date = new Date((candle.time as number) * 1000);
      const formattedDate = date.toISOString().replace('T', ' ').slice(0, 19);
      return `${formattedDate},${candle.open},${candle.high},${candle.low},${candle.close},${candle.volume}`;
    }).join('\n');
    ...
    link.setAttribute('download', `${symbol}_${chartType}_${new Date().toISOString().slice(0,19)}.csv`);
    ...
    <div className="w-full min-h-screen p-4 bg-[#1e1e1e] rounded-lg">
      ...
      <div className="mb-4">
        <button
          onClick={() => {
            if (candleSeriesRef.current) {
              const data = candleSeriesRef.current.data() as ExtendedCandlestickData[];
              saveToCSV(data);
            }
          }}
