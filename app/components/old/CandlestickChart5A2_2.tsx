    ...
    try {
      ...
      if (chartType.startsWith('seconds/')) {
        ...
        candleData = data.map((item: UpbitCandle) => {
          ...
          const kstDate = new Date(item.candle_date_time_kst);
          const timestamp = Math.floor(kstDate.getTime() / 1000) + (2 * 60 * 60);
        ...
        }).reverse(); // 최신 데이터가 마지막에 오도록 역순 정렬

        volumeData = data.map((item: UpbitCandle) => {
          const timestamp = Math.floor(new Date(item.candle_date_time_kst).getTime() / 1000) + (2 * 60 * 60);
          return {
            time: timestamp as Time,
      ...
      chartRef.current.applyOptions({
        localization: {
          timeFormatter: (time: number) => {
            ...
            if (parseInt(chartType) === 86400) { // 년봉
              return `${year}/${month}`;
            } else if (parseInt(chartType) === 7200) { // 월봉
              return `${year}/${month}`;
            } else if (parseInt(chartType) === 240) { // 일봉
              return `${month}/${day}`;
            } else if (chartType.startsWith('seconds/') || parseInt(chartType) <= 60) { // 분봉, 초봉
