...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  // CSV 다운로드 함수: API 호출 시 현재 시간에서 10시간 이전으로 설정하여 CSV 데이터를 가져옵니다.
  const downloadCSV = async () => {
    try {
      const endpoint = getChartEndpoint(chartType);
      const count = getChartCount(chartType);

      // 현재 시간을 기준으로 10시간 빠른(빼는) 시각으로 설정
      const now = new Date();
      ...
      const response = await fetch(`https://api.upbit.com/v1/candles/${endpoint}?market=${symbol}&count=${count}&to=${toTime}`);
      if (!response.ok) {
        throw new Error('CSV 다운로드 API 호출 실패');
      }
      const data = await response.json();
      ...
      link.href = url;
      link.setAttribute('download', `${symbol}_${chartType}_data.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
    } catch (error) {
      console.error('CSV 다운로드 중 에러 발생:', error);
    }
  };
