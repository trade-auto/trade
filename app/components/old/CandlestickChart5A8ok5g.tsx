...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  const handleDateRangeChange = (start: Date) => {
    // 선택한 시간으로부터 30분 후까지의 데이터를 가져오도록 수정
  ...
  const saveToCSV = async () => {
    ...
    try {
      ...
      // API 요청 준비
      const count = 200; // 한 번에 가져올 데이터 수
      let tempData: any[] = []; // 임시 데이터 배열
      let currentDate = new Date(csvDateRange.endDate);
      
      while (currentDate >= csvDateRange.startDate) {
        ...
        try {
          ...
          await new Promise(resolve => setTimeout(resolve, 2000)); // 오류 발생 시 더 긴 대기
          continue;
        }
      }

      if (tempData.length === 0) {
        throw new Error('데이터를 가져오지 못했습니다. 다시 시도해주세요.');
      }
      ...
      const filteredData = tempData
        ...
        .filter(candle => {
          ...
          return candleTime >= csvDateRange.startDate!.getTime() && 
                 candleTime <= csvDateRange.endDate!.getTime();
        });
