...
export const CandlestickChart: React.FC<ChartProps> = ({ symbol, chartType }) => {
  ...
  const findCrossPoints = (thirtyEMA: LineData<Time>[], fortyEMA: LineData<Time>[], sixtyEMA: LineData<Time>[]): CrossPoint[] => {
    ...
    let belowMA60 = false; // 60MA 아래에 있는지 추적
    
    // 초기 시작 시간 설정 (첫 번째 데이터 시간 + 30초)
    const startTime = (thirtyEMA[0].time as number) + 30;
    ...
    for (let i = 1; i < thirtyEMA.length; i++) {
      ...
      // 60MA 아래로 진입하면 플래그 설정
      if (currThirty < currSixty && currForty < currSixty) {
        belowMA60 = true;
      }
      
      // 30MA와 40MA가 60MA 이상이고, 30MA가 60MA를 상향돌파할 때 매수
      if (currThirty > currSixty && currForty > currSixty) {
        if (prevThirty <= prevSixty && currThirty > currSixty && lastAction !== 'buy' && !belowMA60) {
          crossPoints.push({
      ...
      // 30MA와 40MA가 60MA 이하로 떨어질 때 매도
        ...
        lastAction = 'sell';
        belowMA60 = true; // 매도 후 60MA 아래 플래그 설정
      }
    }
  ...
  const handleDateRangeChange = (start: Date) => {
    // 선택한 시간을 그대로 사용
    const endTime = new Date(start.getTime() + 1*60*60 * 60 * 1000); // 30분 후
