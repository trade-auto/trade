...
export async function GET(request: Request) {
  ...
  console.log(`티커 조회 요청: ${market}`);

  try {
    // 업비트 API 호출
    const apiUrl = `https://api.upbit.com/v1/ticker?markets=${market}`;
    ...
    console.log(`티커 조회 성공: ${market}, 가격: ${data[0].trade_price}`);
    return NextResponse.json(data[0]);
  } catch (error: any) {
    console.error('티커 조회 중 오류 발생:', error);
