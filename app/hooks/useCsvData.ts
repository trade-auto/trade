...
interface UseCsvDataReturn {
  ...
  setCsvDateRange: (range: DateRange) => void;
  csvLoading: boolean;
  csvProgress: number;
  ...
  backtestMarkers: SeriesMarker<Time>[];
  csvBacktestResult: BacktestResult | null;
}

export const useCsvData = ({ symbol }: UseCsvDataProps): UseCsvDataReturn => {
  // CSV 상태
    ...
    try {
      ...
      while (currentTo.getTime() > csvFromTime) {
        try {
          ...
          // 진행률 업데이트
          const progressValue = (csvToTime - currentTo.getTime()) / (csvToTime - csvFromTime) * 100;
