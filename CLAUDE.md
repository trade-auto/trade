...
#### PolMACD Ver1.0 매매전략 (PolMACDChart에 구현)
**개요**: 순차적 조건 충족 기반 진짜 매수/매도 신호 전략

**매수 신호 조건 (파란색 화살표, 이평선에 표시)**:
1. **RSI 조건**: RSI(20)가 50을 상향 돌파 (사전 조건)
2. **MACD 조건**: MACD 라인이 Signal 라인을 상향 돌파 (사전 조건)
3. **진짜 매수 신호**: RSI와 MACD 조건이 만족된 후 20 EMA가 60 EMA를 상향 크로스
4. **200 EMA 필터**: 200 EMA가 10봉 연속 음의 기울기이면 매수 금지
5. **매수/매도 순서**: 초기는 매수부터 시작, 이후 매수는 매도 신호 이후에만 가능

**매도 신호 조건 (빨간색 화살표, 이평선에 표시)**:
1. **포지션 보유**: 이전 매수 신호 이후 보유 상태
2. **진짜 매도 신호**: MACD가 신호선 아래로 가는 시점 + MACD 신호선 음의 기울기 10봉 완성 시점

**신호 표시**:
- 진짜 매수/매도 신호는 캔들 차트의 이평선 영역에 표시
- 매수: 파란색(#0000FF) 상향 화살표
- 매도: 빨간색(#FF0000) 하향 화살표
- 개별 조건 신호들은 보라색 계열로 각 차트에 분리 표시

**기술적 지표**:
- EMA: 5, 20, 60, 200
- MACD: 표준 12-26-9
- RSI: 20 기간
- 차트: 3개 분리형 (캔들+EMA, MACD, RSI)

**백테스트**: 수수료 0.1% (왕복) 자동 반영

- **EMA-MACD-RSI 추세추종 전략** (이전 버전):
